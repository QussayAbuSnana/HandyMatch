...
export default function ProProfilePage() {
  ...
  const menuItems = [
    { title: t("manage_services"), icon: Wrench, iconWrap: "bg-violet-50", iconColor: "text-violet-600", href: "/pro/profile/services" },
    { title: t("availability_label"), icon: Clock3, iconWrap: "bg-blue-50", iconColor: "text-blue-600", href: "/pro/availability" },
    { title: t("earnings_label"), icon: Wallet, iconWrap: "bg-emerald-50", iconColor: "text-emerald-600", href: "/pro/profile/earnings" },
    { title: t("settings"), icon: Settings, iconWrap: "bg-gray-100", iconColor: "text-gray-500", href: "/pro/profile/settings" },
  ];
    ...
    <main className="min-h-screen bg-[#f8f8fb] pb-28">
      ...
      <header className="sticky top-0 z-30 border-b border-gray-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5">
          ...
          <h1 className="text-3xl font-bold text-slate-900">{t("my_profile")}</h1>
        ...
        <div className="rounded-[2rem] bg-gradient-to-r from-violet-600 to-fuchsia-500 p-8 text-white shadow-sm">
          <div className="flex items-center justify-between gap-5">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="mt-1 text-xl text-white/85">{t("pro_since")} {memberSince}</p>
                {pro.isAvailable !== undefined && (
                  <span className={`mt-2 inline-block rounded-full px-4 py-1 text-lg font-semibold ${pro.isAvailable ? "bg-green-400/30 text-green-100" : "bg-white/20 text-white/70"}`}>
                    {pro.isAvailable ? t("available_badge") : t("unavailable")}
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-3 gap-6 text-center">
            <div>
              ...
              <div className="mt-2 text-lg text-white/85">{t("rating_label")}</div>
            </div>
            <div>
              <div className="text-4xl font-bold">{pro.jobCount ?? 0}</div>
              <div className="mt-2 text-lg text-white/85">{t("jobs_done_label")}</div>
            </div>
            <div>
              <div className="text-4xl font-bold">{pro.reviewCount ?? 0}</div>
              <div className="mt-2 text-lg text-white/85">{t("reviews")}</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <section className="mx-auto max-w-7xl px-5 pt-6">
          <div className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-3xl font-bold text-slate-900">{t("services")}</h2>
              <span className="text-2xl font-bold text-violet-600">${pro.hourlyRate ?? "—"}{t("hr")}</span>
            </div>
      ...
      {reviews.length > 0 && (
        <section className="mx-auto max-w-7xl px-5 pt-6">
          <div className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm">
            <h2 className="mb-5 text-3xl font-bold text-slate-900">{t("customer_reviews")}</h2>
      ...
      <section className="mx-auto max-w-7xl px-5 pt-6">
        <div className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="space-y-5">
            {[
              { icon: Mail, labelKey: "email", value: userProfile?.email ?? "—" },
              { icon: Phone, labelKey: "phone", value: pro.phone ?? t("not_set") },
              { icon: MapPin, labelKey: "location", value: pro.location ?? t("not_set") },
              ...
              <div key={labelKey} className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-lg text-slate-500">{t(labelKey)}</p>
      ...
      <section className="mx-auto max-w-7xl px-5 pt-8">
        <button onClick={handleLogout}
          ...
          <LogOut className="h-6 w-6" />{t("log_out")}
        </button>
      </section>
