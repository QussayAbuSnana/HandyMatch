...
export default function ProServicesPage() {
  ...
  function addService() {
    ...
    setServices((prev) => [...prev, { id: Date.now(), name: newService.trim(), price: newPrice.trim() }]);
    setNewService("");
    setNewPrice("");
    setSaved(false);
  }
      ...
      <section className="bg-gradient-to-r from-indigo-600 via-violet-600 to-pink-500 px-5 pb-8 pt-6 text-white">
        <div className="mx-auto max-w-4xl">
          <div className="mb-6 flex items-center justify-between">
            ...
            <div className="rounded-full bg-white/15 px-4 py-2 text-sm font-semibold">{t("manage_services")}</div>
          </div>
          <p className="text-lg text-white/85">{t("pro_profile_settings")}</p>
          <h1 className="mt-2 text-4xl font-extrabold md:text-5xl">{t("manage_your_services")}</h1>
          <p className="mt-3 text-lg text-white/85 md:text-xl">{t("manage_services_desc")}</p>
        </div>
      </section>
      ...
      <section className="mx-auto -mt-4 max-w-4xl px-5">
        <div className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <textarea
            ...
            placeholder={t("describe_services_ph")}
            dir="auto"
        ...
        <div className="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid gap-4 md:grid-cols-[1fr_180px]">
            <input
              type="text"
              value={newService}
              onChange={(e) => { setNewService(e.target.value); setSaved(false); }}
              placeholder={t("service_name_ph")}
              dir="auto"
              className="w-full rounded-[1.2rem] border border-gray-200 bg-white px-4 py-4 text-lg text-slate-700 outline-none transition placeholder:text-gray-400 focus:border-violet-400"
            />
            <input
              type="number"
              value={newPrice}
              onChange={(e) => { setNewPrice(e.target.value); setSaved(false); }}
              placeholder={t("price")}
              className="w-full rounded-[1.2rem] border border-gray-200 bg-white px-4 py-4 text-lg text-slate-700 outline-none transition placeholder:text-gray-400 focus:border-violet-400"
            />
          </div>
        ...
        {saved && (
          <div className="mt-5 rounded-[1.5rem] border border-emerald-200 bg-emerald-50 p-5 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-slate-900">{t("services_updated")}</h3>
                <p className="mt-1 text-slate-600">{t("services_updated_desc")}</p>
              </div>
            </div>
          </div>
        )}
